...
%% Sound Seed
S.Seed.FolderParent =   'D:\SynologyDrive\=sounds=\Vocalization\LZ_selected_2s\single rep\';
...
% Session, % Trial
    ...
    S.Session.SeedSeq =     13 + mod(randperm(3*32), 3); % total 4 * CycleNum
    S.Trial.Names =         {'VocMatchST_full_TW'}; % Model Matched
    S.Seed.TotalTime =      2.375;
...
list = dir(fullfile([S.Seed.FolderParent],'*.wav'));
...
%     S.Seed.SoundRS{i,2} = S.Seed.SoundRS{i,2}/std(S.Seed.SoundRS{i,2});
   [S.Seed.SoundSpec{i}, ~] = audioread([ S.Seed.FolderParent,...
        '\ModelMatched_sounds\',...
        S.Seed.TitleSeeds{i,1}, '_0_synth.wav']);
    S.Seed.SoundRS{i,2} = resample(S.Seed.SoundSpec{i}, S.System.SR, S.Seed.FS);
    S.Seed.SoundRS{i,2} = S.Seed.SoundRS{i,2}/std(S.Seed.SoundRS{i,2});
