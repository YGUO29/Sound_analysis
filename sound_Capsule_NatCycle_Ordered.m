% Cycle-based Natural sound vs. Control Natural sound
% the order of sound is pre-determined (by similarity of responses, for
% example)
clear all

%% Sound Seed
S.Seed.FolderParent =   'D:\SynologyDrive\=sounds=\Natural sound\';
...
% Session, % Trial
    ...
    load outperm
    S.Session.SeedSeq =     repmat(outperm, 1, 4); % music/speech total 52 sounds
    S.Trial.Names =         {'NatMatchST_full_4SeedsSame'};
    S.Seed.TotalTime =      2.25;
    S.Seed.PreStimTime =    0.25;  
    
%     S.Session.CycleNum =	100;
% %     S.Trial.ModSeqSpec =	[1 4 2 3];
%     S.Trial.ModSeqSpec =	[4 1 3 2];
    
    S.Session.CycleNum =	52;
%     S.Trial.ModSeqSpec =	[6 8 1 3 5 7 2 4];
    S.Trial.ModSeqSpec =	[1 3 5 7 2 4 6 8];
...
% get a list of sound names (natural sounds)
list = dir(fullfile([S.Seed.FolderParent, 'Natural_JM_ModelMatched_Sounds\MusicSpeech\'],'*.wav'));
[names_sound, idx] = natsortfiles({list.name});
list = list(idx);
% list = [list(1:52); list(1:52); list(1:52); list(1:52)];

for i = 1:52
    % for 165 sounds: change i to i+1, get rid of the first sound alarm
    % clock, which generated very week responses
    [~,S.Seed.TitleSeeds{i,1},~] = fileparts(list(i).name);
%     S.Seed.TitleSeeds{i,1} = list(i).name; % 
...
%     S.Seed.SoundRS{i,1} = S.Seed.SoundRS{i,1}/std(S.Seed.SoundRS{i,1});
   [S.Seed.SoundOrig{i}, S.Seed.FS] = audioread([ S.Seed.FolderParent,...
        'Natural_JM original\',...
        S.Seed.TitleSeeds{i,1}(1:end-8), '.wav']);
    S.Seed.SoundRS{i,1} = resample(S.Seed.SoundOrig{i}, S.System.SR, S.Seed.FS);
    S.Seed.SoundRS{i,1} = S.Seed.SoundRS{i,1}/std(S.Seed.SoundRS{i,1});
...
%     S.Seed.SoundRS{i,2} = S.Seed.SoundRS{i,2}/std(S.Seed.SoundRS{i,2});
   [S.Seed.SoundSpec{i}, ~] = audioread([S.Seed.FolderParent,...
        'Natural_JM_ModelMatched_Sounds\MusicSpeech\',...
        S.Seed.TitleSeeds{i,1}, '.wav']);
    S.Seed.SoundRS{i,2} = resample(S.Seed.SoundSpec{i}, S.System.SR, S.Seed.FS);
    S.Seed.SoundRS{i,2} = S.Seed.SoundRS{i,2}/std(S.Seed.SoundRS{i,2});
...
S.Session.Artist =      ['Yueqi @ ' datestr(now, 'yyyymmdd-HH')];
...
for i = 1:S.Session.CycleNum
    ...
    for i2 = 1:size(S.Session.SeedMat, 2) % number of seed per cycle
        k = S.Session.SeedMat(i, i2);
        for j = 1:2 % 1=Orig, 2=Control sound
            % slot number
            k2 = j+(i2-1)*2;
...
%                 S.Seed.SoundRScut{k,j};   
            S.Seed.Sound( S.Seed.SampleBase+(1:length(S.Seed.SoundRS{k,j})) ) = ...
                S.Seed.SoundRS{k,j};   
...
switch S.Trial.SeedNum
    ...
    % Spec
    SeedOrder = num2str(S.Trial.ModSeqSpec); 
    SeedOrder = SeedOrder(find(~isspace(SeedOrder)));

    S.Session.Title = [...
        sprintf('PreA_%s_',         S.Trial.Names{1}),...
        sprintf('Order=%s_',        SeedOrder), ...
        sprintf('%dx',       S.Session.CycleNum),...
