...
function F = getFeatures_singlefile(Sd, opt)
...
%     mode, plotON)
    ...
    F.CochEnv_ds_log = CochEnv_ds_log;
    % ===================================================================
    F.cf_log = f2;
...
%     M = all_filter_moments_from_coch(F.CochEnv_ds', P, 1:size(F.CochEnv_ds',1));
    ...
    F.spectemp_mod_weighted_full = Y; % keep +/- temporal rates
    F.spectemp_mod_weighted = squeeze(mean(spectemp_mod_weighted,3));
   
    % ======= ========================== =======
    if opt.plotON   
        % spectral profile
        subplot(2,3,2)
        semilogy(F.coch_env, F.cf_log, 'LineWidth',3), 
        xlim([min(F.coch_env), max(F.coch_env)])
        ...
        subplot(2,3,3)
        imagesc(F.temp_mod'), axis('xy'), colorbar
        ...
        subplot(2,3,4)
        imagesc(F.spec_mod'), axis('xy'), colorbar
        freqs_to_plot = [100 400 800 1600 3200 6400];
        fticks = floor(interp1(P.f, 1:1:length(P.f), freqs_to_plot));
        set(gca, 'YTick', fticks, 'YTickLabel', (freqs_to_plot)/1000);
