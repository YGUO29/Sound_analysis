...
function detectedCalls = call_detect_parabolic_xhw( BehChannel,DetectionParam )
    ...
    for ci = 1 : Nfor
        ...
        parGain                 =   BehChannel.SigDetect.get_gain(parCallSpe,'spe');
        refGain                 =   BehChannel.SigRef.get_gain(refCallSpe,'spe');
        ...
        for iEvent = 1 : eventCounter
            ...
            callEnergy = mean(max(parCallSpeRaw(band(1):band(2) , startP : stopP)));
            k = 3;
            if callEnergy < param.callEnergyLB
                continue;
            end
            expandThreshold = max(min(callEnergy - param.expandBoundaryGapOut, param.callEnergyUB),param.callEnergyLB);
            while (startP > p2f) && (mean(max(parCallSpeRaw(band(1):band(2),startP - p2f : startP + k * p2f))) > expandThreshold)
                startP = startP - p2f;
            end
            while (stopP < size(parCallSpeRaw,2)-p2f) && (mean(max(parCallSpeRaw(band(1):band(2),stopP - k * p2f : stopP + p2f))) > expandThreshold)
                stopP  = stopP + p2f;
            end
        ...
        timeStartTemp   =   (indStartTemp) * param.secPerFrameInd;
        timeStopTemp    =   (indStopTemp) * param.secPerFrameInd + param.secBaseline;
        timeTemp1        =   [timeStartTemp; timeStopTemp];
        indStartTemp1   =   [];
        indStopTemp1    =   [];
        for iEvent = 1 : eventCounter
            if (flag(iEvent) == 1)
...
% --------------------------
            ...
            elseif...
                ...
                (timeStopTemp(iEvent) - timeStartTemp(iEvent) > param.minCallDuration)&&...
                (parEnergy > param.parEnergyThreshold)&&...
                (AHR < param.AHRThresholdUB)&&(AHR > param.AHRThresholdLB)
