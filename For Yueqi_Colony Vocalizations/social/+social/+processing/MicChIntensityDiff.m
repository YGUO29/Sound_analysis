function [out, param_out] = MicChIntensityDiff(y_sig1,y_sig2,param_in)
...
% %     if isempty(frame_win)
        ...
        if strcmp(param_in.mode,'file')
            ...
            if strcmp(param_in.task,'detect')
                ...
                voc_detect_band = [4000 18000]; % in Hz, frequency range for call detection
            ...
            elseif strcmp(param_in.task,'GetCallF0')    % get some parameters from the parent function
                ...
                voc_detect_band = [1000 20000]; % in Hz, frequency range for call detection
            end
        ...
        spec_avg_win = hamming(round(1000/(1/frame_win)));       % 1000 for detection, 500 for viewing
    %     spec_avg_win = hamming(3);
        spec_avg_win = spec_avg_win/sum(spec_avg_win);
        time_avg_win = hamming(30);                              % 30 for detection, 6 for viewing
...
%     end
        ...
        [spec{1},x,y] = social.util.spectra(y_sig1,winsize,round(shift_time*Fs),Fs,'log','gausswin');
        [spec{2},x,y] = social.util.spectra(y_sig2,winsize,round(shift_time*Fs),Fs,'log','gausswin');
...
%     [spec_diff,x,y] = social.util.spectra(y_sig1-y_sig2*10^(spec_th/20),winsize,round(shift_time*Fs),Fs,'log','gausswin');
    
    if ~isempty(spec_diff)  % in case chunk is too small, no "spec" is calculated
       
        for j = 1:size(spec_diff,2)
            spec_diff_smooth(:,j) = filtfilt(spec_avg_win,1,spec_diff(:,j));
        end
        for j = 1:size(spec_diff,1)
