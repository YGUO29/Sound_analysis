% added additional sessions from 4 Monkey parabolic mic recordings for
% training

% manually verified selection table is in the "For Feature Extraction"
% folder (moved to "groundTruth" folder)

parabolic_range = 64:134;       % session range where parabolic mic is used, may not exist in the Excel table
...
if (exist(fullfile(dataFolder, param.HMMParamFileName), 'file') == 2) && (retraining == 0)
    ...
    elseif (exist(fullfile(dataFolder, ['call_seg',param.HMMParamFileName(9:end)]), 'file') == 2) && (rebuilding == 0)
        ...
        elseif strcmp(param.HMMParamFileName(10:end-4), 'only4M')
            ...
            % it should also read from the Done folder to include selection
            % tables
            flist = dir([param.selectionTablePath,'\*',subject,'*.txt']);
            N_f = length(flist);
            for i = 1:N_f
                thisname = flist(i).name;
                ind1 = strfind(thisname,'_S');
                ind2 = strfind(thisname,'.txt');
                p_ses = str2num(thisname(ind1+2:ind2-1));      
                if ~isempty(p_ses) && isempty(find(BehChns(:,1)==p_ses))
                    for cc = 1:4
                        Beh_row = [p_ses,cc,cc,5,cc];
                        BehChns = [BehChns;Beh_row];
                    end
                end
            end
            ...
            for i = 1 : length(BehChns)
                ...
                refChannel          =   BehChns(i,4);
                boxChannel          =   BehChns(i,5);
%                 boxChannel          =   parChannel;     % for debug, L.Zhao
                sampleProvider      =   social.vad.SampleProvider(fileName, tableName, nTableName, parChannel, refChannel, boxChannel, param);
                call_seg_temp       =   sampleProvider.extractSegs('call', 'parData', 1);
                if isempty(call_seg)
                    call_seg        =   call_seg_temp;
        ...
        save(fullfile(param.dataFolder,['call_seg',param.HMMParamFileName(9:end)]),'call_seg','-v7.3');
    end    
    % ------------------end----------------------
