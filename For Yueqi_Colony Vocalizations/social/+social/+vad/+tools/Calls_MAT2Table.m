...
for i = 1:length(beh)
    for j = 1:length(beh(i).Events)
        ...
        fprintf(fid,'%d\t%s\t%d\t%f\t%f\t%f\t%f\t%f\t%s\t%f\t%f\t%f\t%f\t%f\t%f\r\n',...   
                    k,...
                    'Spectrogram 1',...
                    beh(i).Events(j).channel,... 
                    beh(i).Events(j).eventStartTime,...  
                    beh(i).Events(j).eventStopTime,... 
                    4000,...
                    18000,...
                    8000,...
                    beh(i).Events(j).eventPhraseType,... 
                    beh(i).Events(j).energyDiff,...
                    beh(i).Events(j).energyPar,...
                    beh(i).Events(j).energyDiffWireless,...
                    beh(i).Events(j).energyParWireless,...
                    beh(i).Events(j).AHR,...
                    beh(i).Events(j).exception...
               );
        k = k + 1;
    end
end
