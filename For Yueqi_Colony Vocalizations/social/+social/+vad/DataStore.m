...
classdef DataStore<handle
    ...
    methods (Access=public)
        ...
        %   Haowen Xu, 08/2016
        function  prepareVAD(self,callFrame,noiseFrame,trainTestRatio )
            self.valueType = social.vad.ValueType.gaussian;
            nCall = length(callFrame);
            nNoise = length(noiseFrame);
            nTrainCall = floor(nCall*trainTestRatio);
            nTestCall = nCall - nTrainCall;
            nTrainNoise = floor(nNoise*trainTestRatio);
            nTestNoise = nNoise - nTrainNoise;
            if ~isempty(callFrame)
                [M,N] = size(callFrame{1,1});
                L = M * N;
            else
                [M,N] = size(noiseFrame{1,1});
                L = M * N;
            end
            
            for i = 1 : length(callFrame)
                callFrame{i} = reshape(callFrame{i}, 1, L);
            end
            for i = 1 : length(noiseFrame)
                noiseFrame{i} = reshape(noiseFrame{i}, 1, L);
            end
            if iscell(callFrame)
            %   add call frame to data
                self.trainData = cell2mat(callFrame(1:nTrainCall));
                self.trainLabels = ones(nTrainCall,1);

                self.testData = cell2mat(callFrame(nTrainCall+1:nCall));
                self.testLabels = ones(nTestCall,1);
            else
                self.trainData = callFrame(1:nTrainCall,:);
                self.trainLabels = ones(nTrainCall,1);

                self.testData = callFrame(nTrainCall+1:nCall,:);
                self.testLabels = ones(nTestCall,1); 
            end

            if iscell(noiseFrame)
            %   add noise frame to data
                self.trainData = [self.trainData; cell2mat(noiseFrame(1:nTrainNoise))];
                self.trainLabels = [self.trainLabels; zeros(nTrainNoise,1)];

                self.testData = [self.testData; cell2mat(noiseFrame(nTrainNoise+1:nNoise))];
                self.testLabels = [self.testLabels; zeros(nTestNoise,1)];
            else
                self.trainData = [self.trainData; noiseFrame(1:nTrainNoise,:)];
                self.trainLabels = [self.trainLabels; zeros(nTrainNoise,1)];

                self.testData = [self.testData; noiseFrame(nTrainNoise+1:nNoise,:)];
                self.testLabels = [self.testLabels; zeros(nTestNoise,1)];        
            end
        end
        ...
        function normalize(obj,method)
            ...
            if (strcmp(method,'minmax'))
                ...
                obj.valueType=social.vad.ValueType.probability;
            ...
            if (strcmp(method,'meanvar'))
                ...
                %changed.
                obj.valueType=social.vad.ValueType.gaussian;
        ...
        function batchSize=computeBatchSize(obj)
            ...
            if (ispc && ~social.vad.tools.isoctave())
