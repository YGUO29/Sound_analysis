function [t_start,t_stop] = findphee_mvx(wave, chunk_start_time, th, Fs, filt)
...
% th: threshold
    ...
    param.min_phee_len = 0.4;      % minimum phee phrase duration
    param.th = th;
    param.start_time = chunk_start_time;
    param.pre_th_len = 0.2;         % time period before crossing threshold to search for phee start
...
%     plot([1 length(wave_power)],[1 1]*param.th,'r')
    ...
    param.reversed = 0;
    t_start = FindPheeStart(wave,wave_unfiltered,wave_power,param);
...
%     t_stop = sort(t_stop);
    t_stop = FindPheeStop(wave,wave_unfiltered,wave_power,param,t_start);
...
function out = InPheeBand(y,Fs_down)
    ...
    target_range = [6000 9000];   % frequency range (Hz) to look at
    ...
    if pheeband > refband * 2 && pheeband > refband2 * 2
        out = 1;
    else
        out = 0;
    end
...
function t_start = FindPheeStart(wave,wave_unfiltered,wave_power,param)
    ...
    win_size = round(0.005*Fs_down/2)*2;
        ...
        for i = 1:length(checkpoints)
            if scanned(i) == 0
                ...
                if checkpoints(i)-scanpoints-0.1*Fs_down > 0
                    PreIsPhee = InPheeBand(wave_unfiltered(checkpoints(i)-scanpoints-round(0.1*Fs_down):checkpoints(i)-round(0.1*Fs_down)-1),Fs_down);
                else
                    PreIsPhee = 0;
                    PostIsPhee = 1;
                end
                ...
                if IsPhee && ~PreIsPhee && PostIsPhee
...
function t_stop = FindPheeStop(wave,wave_unfiltered,wave_power,param,t_start)
...
%                             && (pos < (round(0.005*Fs_down/2)+1)/(Fs_down/2)*stop_range(1)))
    ...
    %                                 timelabel(j) = 1;
                        ...
                        if stop_temp+startcheck+round(0.1*Fs_down) < length(wave)
                            PostIsPhee = InPheeBand(wave_unfiltered(startcheck+stop_temp:startcheck+stop_temp+round(0.1*Fs_down)-1),Fs_down);
                        else
                            PostIsPhee = 0;
                        end
                        if ~PostIsPhee
