function [feature] = Tfeature_ana_DFA_paper(type, data, visu, varargin)
% To analyze acoustic feature in each call type
% This is derived from Tfeature_ana from the same folder
% Tfeature_ana in this folder seems to be updated from Dana's original Tfeature_ana
...
%         temp_rowidx2(pot_noiseidx) = 66;
        ...
        negtrough = negtrough(negtrough>1);  % fix index = 1 issue, by L.Zhao 2018.07
...
%         if feature.tphr_e > 0.065 || feature.tphr_e < 0.005, feature = []; return; end
        ...
        if feature.fbw_b <= 1 || feature.fbw_m < 1 || feature.fbw_e <= 0, feature = []; return; end 
