function [feature] = Tfeature_ana_Dana_fix(type, data, visu)
...
%             if feature.tphr_e > 0.065 || feature.tphr_e < 0.005, feature = []; return;end
    ...
    case 'phee'
        if ~isempty(xcol5)
            ...
            if feature.dur < 0.1, feature = []; return;end
            if feature.fmin < 4, feature = []; return;end
            if feature.fdom < 4, feature = []; return;end
            if feature.fmax >= 11, feature = []; return;end
        else
            feature = [];return;
        end
    ...
    case 'tril'
        if ~isempty(xcol5) && ~(xrow5(1) == 66 && isempty(xrow5(diff(xrow5)>0)))
            ...
            if ~(isempty(peak) || isempty(trough))
                ...
                if isnan(feature.FMrate) || feature.FMrate > 60, feature = []; return;end
    ...
    case 'trph'
        if ~isempty(xcol5)
            ...
            if ~(isempty(peak) || isempty(trough) || isempty(trans))
                ...
                if isnan(feature.FMrate) || feature.FMrate > 70, feature = [];return; end
                if feature.tTrans >= 1 || feature.tTrans <= 0, feature = [];return; end
            else
                feature = [];
            end
            if feature.dur < 0.1, feature = [];return; end
