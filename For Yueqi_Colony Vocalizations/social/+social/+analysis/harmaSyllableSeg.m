function [syllables,FS,S,F,T,P] = harmaSyllableSeg(signal,FS,WINDOW,NOVERLAP,NFFT,MINDB,FBAND,cutoff)
...
% Window spectrogram by FBand
if ~isempty(FBAND)
    % Find frequencies outside of the call frequency band
    exclude_inds=find(F<FBAND(1)|F>FBAND(2)); 
    S(exclude_inds,:)=[];
    P(exclude_inds,:)=[];
    F(exclude_inds)=[];
end

%Compute magnitude of spectrogram
mag = abs(S);

figure(10);
ah=subplot(3,1,1);
hist(20.*log10(mag(:)),300);
hold('on');
plot(mean(20.*log10(mag(:)))*[1 1],ah.YLim,'r');
plot(mean(20.*log10(mag(:)))*[1 1]+6*std(20.*log10(mag(:))),ah.YLim,'r');

cutoff=mean(20.*log10(mag(:)))+cutoff*std(20.*log10(mag(:)));

ah1=subplot(3,1,2);
% inds=20.*log10(mag)>cutoff;
% data=P;
% data(~logical(inds))=nan;
% data(logical(inds))=1;
surf(T,F,100+10*log10(abs(P)),'EdgeColor','none');
hold on;
lh(1)=plot(min(T)*[1 1],[min(F) max(F)],'k');
lh(2)=plot(max(T)*[1 1],[min(F) max(F)],'k');
hold off;
...
while(1)
    ...
    % TODO SDK Add constraint to look within +/- 1/2 octave.
    t = segmentIndex;
...
%     lh(1).XData=[1 1].*times(1);
%     lh(2).XData=[1 1].*times(end);
%     xlim(times([1 end])+[-2 2]);
%     title(['Syllable ' num2str(N) ': ' num2str(times(1)) ' - ' num2str(times(end)) ' s']);
%     pause(1);
