...
% th: threshold
    ...
    param.min_interphrase_interval = 0.1;       % min time between phrases
...
function out = InPheeBand(y,Fs_down)
    ...
    scanpoints = length(y);
    f_range=linspace(-Fs_down/2,Fs_down/2,scanpoints);

    y_fft = abs(fft(y));     % total length is scanpoints
    temp_fft=y_fft;
    temp_fft(~(f_range>target_range(1)&f_range<target_range(2)))=0;
    [peak,peak_freq]=max(temp_fft);
%     figure(15); plot(f_range,y_fft); hold on; plot(f_range,temp_fft,'r'); plot(f_range(peak_freq-5:1:peak_freq+5),ones(11,1).*peak,'g');    
    pheeband = mean(y_fft(max(1,peak_freq-5):min(peak_freq+5,length(y_fft))));
    ...
    if pheeband > refband * 2 %Threshold decreased by SDK on 3/22/14
        out = 1;
    else
        out = 0;
    end
            ...
            if scanned(i) == 0
                ...
                if IsPhee && PostIsPhee % && ~PreIsPhee 
                    ...
                    if checkpoints(i) == 1
                        ...
                        spec = social.analysis.spectra(wave(startcheck:endcheck),round(win_size),round(spec_shiftlen*Fs_down),Fs_down,'linear');
                        if i == 47
...
function t_stop = FindPheeStop(wave,wave_unfiltered,wave_power,param,t_start,checkpoints)
    stop_range = [5000 10000];      % phee frequency band used to judge stop time
    Fs_down = param.Fs_down;
    scanlen = param.scanlen;
    spec_shiftlen = param.spec_shiftlen;
    min_phee_len = param.min_phee_len;
