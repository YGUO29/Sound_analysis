% Directional Detecting with a parabolic mic and a shotgun mic
...
% ch_shotgun = 4;          % channel for shotgun mic as a reference
...
ch_shotgun = 2;          % channel for shotgun mic as a reference
IsRunning = 1;           % a flag to indicate the program is running
...
param.ch_ref = ch_shotgun;
% buffer_length = 0.02;    % buffer length
wavebuffer = [];         % a matlab buffer to store a temporary fragment of data for computing fine spectrogram
...
overlap = 0;
voc_max_band = 1000;     % in Hz, maximum instantaneous bandwidth of a call
...
for si = 1:length(session_list)
    ...
    try
        ...
        % for ci = Nstart_debug:Nfor
            ...
            y_sig = y_temp(:,[ch_parabolic,ch_shotgun]);
