function plotSpectempIRF
% modified based Sam's code plot_spectrotemporal_IRFs.m

clc;
close all;

% find the directory containing this file
name_of_this_file = 'plotSpectempIRF';%mfilename;
directory_containing_this_file = fileparts(which(name_of_this_file));

% McDermott Texture toolbox
addpath(genpath('D:\=code=\McdermottLab\toolbox_Sound_Texture_Synthesis_v1.7\Sound_Texture_Synthesis_Toolbox'));

% color maps
addpath(genpath(['D:\=code=\Toolbox\FileExchange\cbrewer']));
...
temp_mod_rates_pos_and_neg = ...
    [fliplr(-P.temp_mod_rates(P.temp_mod_rates>0)), P.temp_mod_rates];
...
for i = 1:length(spec_mod_rates)
    for j = 1:length(temp_mod_rates_pos_and_neg)
        ...
        % plot image
%         figure;
        subplot(length(spec_mod_rates), length(temp_mod_rates_pos_and_neg), (i-1)*length(temp_mod_rates_pos_and_neg)+j)
