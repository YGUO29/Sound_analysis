...
bnum = 21;
F.PathOutput = 'D:\SynologyDrive\=sounds=\Natural sound\Exp_Natural_JM_ModelMatched\';
...
% ==== note ====
% unattenuated level = 100dB 
% (sine wave with max value 1, which has power 0.5)
% any sound that already have power x, it's already attenuated by:
% 10 * log10(x/0.5)
% additional attenuation should be desired attenuation (relative to 100)
% plus the value (negative) calculated above
S.D.Level.att =             0; % actual attenuation ~57dB 
S.D.Trial.Duration =        18;
S.D.Trial.PreStimTime =     0;
S.D.Trial.Repeat =          1;
S.D.Trial.SR =              100000;
S.D.Trial.Resample =        1;
S.D.Trial.ResampleLength =  18; % length of single sound loaded
S.D.Trial.CycNum =          1;
S.D.Trial.OrderMat =        mat2str(ones(1, length(F.FileName)));
...
S.D.SoundT.Total =          zeros(1, floor(S.D.System.SR* S.D.Trial.Duration));
...
S.T.Sound.Total =           zeros(1, floor(S.D.System.SR*length(F.FileName)*S.D.Trial.Duration));
...
%     S.T.Trial.Names{i} =	[num2str(S.T.NumH), '=', num2str(S.T.NumV)];
    ...
    S.T.Trial.Names{i} =    S.T.Trial.Names{i}(end-6:end);
%     S.T.Trial.Names{i} = 'S23s&T19s';
    ...
    S.T.Sound.Totaluint16((i-1)*floor(S.D.System.SR*S.D.Trial.Duration)+1 : floor(i*S.D.System.SR*S.D.Trial.Duration)) = S.D.SoundT.CurWave;
...
% automatic name (temporary for natural sounds)
% ind_Cat = strfind(F.FileName{1}, 'Cat=');
% name_temp = F.FileName{1}(ind_Cat+4:ind_Cat+14);

S.D.Session.Title = ['Nat_', ...
    'ModelMatched_Full_',...    
    sprintf('%d',   S.D.Trial.NumberTotal),     'sounds_(',...
    sprintf('%2.1f',S.D.Trial.PreStimTime),     'pre_in_',...
    sprintf('%2.1f',S.D.Trial.Duration),        ')s_',...
    num2str(S.T.Trial.Att(1)-10*log10(1e-4/0.5), '%2.1f'), 'dBatt'];
...
S.D.Session.Comment = [' ',...
    ...
    ''];
audiowrite([F.PathOutput, S.D.Session.Title,'.wav'],...
